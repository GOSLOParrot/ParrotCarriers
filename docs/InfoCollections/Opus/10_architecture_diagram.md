...
> 基于全部调研结论 (ADR-001 ~ ADR-032) 和核心结论 (1~42) 重新绘制
...
| 7 | **L1 主发现路径修正**: 明确 SAM2全分割为发现主路径，YOLO-World 作为可选兜底。 | 本次对话 |
...
|:-----|:---------|:---------|:-------|:---------|
| **L1** | 视网膜 (Retina) | `List[Detection]` 含三层缓冲 | YOLO-World + SAM2 + DINOv2 | ≤30fps (Tier 依赖) |
