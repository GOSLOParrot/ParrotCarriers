...
*   [🎨 Design Patterns Detected](#🎨-design-patterns-detected)
...
-   `agent/loop.py`: Implements the main LLM ↔ tool execution cycle.
...
1.  **Agent Loop Design (`agent/loop.py`)**: Focus on the core LLM ↔ tool execution cycle. This is a primary reference for understanding how `nanobot` workers are designed and can inform the design of your own `nanobot`-based worker processes.
