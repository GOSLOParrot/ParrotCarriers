...
def _node_payload(node: Any) -> dict[str, Any]:
    return {
        ...
        "graphiti_uuid": node.graphiti_uuid,
        "obsidian_uuid": node.obsidian_uuid,
        "category": node.category,
        "description": node.description,
        "known_facts": list(node.known_facts or []),
        "tags": list(node.tags or []),
        "typical_location": node.typical_location,
        ...
        "provenance_stream_id": node.provenance_stream_id,
        "time_span": list(node.time_span or (0.0, None)),
        "reference_image_path": node.reference_image_path,
        "last_sighting_path": node.last_sighting_path,
...
def _edge_payload(src: Any, dst: Any, edge: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "view_classes": list(getattr(edge, "view_classes", ()) or meta.get("view_classes", ())),
        "graphiti_uuid": getattr(edge, "graphiti_uuid", ""),
        "source_graphiti_uuid": getattr(edge, "source_graphiti_uuid", ""),
        "target_graphiti_uuid": getattr(edge, "target_graphiti_uuid", ""),
        "ref_ids": list(getattr(edge, "ref_ids", ()) or meta.get("ref_ids", ())),
        "meta": meta,
    }
