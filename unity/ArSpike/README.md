...
1. **AR 基线探针**（原始角色）：AR Mobile Template + AR Foundation 5.1.x，用于平面检测 / 放置等 AR 基线验证，是 `.cursor/skills/ar-foundation-api` 与 `ar-foundation-samples` 的对照实现。
...
- **模板默认**：通过 Unity Hub 创建该工程时，Package Manager 会为 AR Foundation / ARCore / ARKit 解析到 **5.2.0**（示例：`Packages/manifest.json` 曾一度为 `5.2.0`）。
- **本仓库改动**（与 ParrotCarriers 对齐）：三个包已**显式锁为 5.1.5**，与 `unity/ParrotDev` 及 `.cursor/skills/ar-foundation-api`（Unity 2022.3 + AR Foundation **5.1.x**）一致。
...
- **不要在 manifest 里显式钉死**该包，除非你有明确理由。模板曾带 `4.5.0` 顶层依赖，而 **AR Foundation 5.1.x** 在包元数据里对 `xr.management` 的要求是下界（锁文件里常写成 `4.0.1`）；Unity UPM 会在**单一版本**上收敛，钉 `4.5.0` 与主工程 **不钉、解析为 4.4.0**（`ParrotDev`）容易造成团队间解析结果不一致，偶发 Package Manager 警告或与编辑器补丁行为差异。
