...
| L1.5/L2-B operator drafts | `GET /api/l15/pool`, `GET /api/l15/obsidian-vault/scan`, `POST /api/l15/obsidian-vault/import-draft`, `POST /api/l15/obsidian-vault/import`, `POST /api/l15/bucket-op/draft`, `POST /api/l15/bucket-op`, `POST /api/l15/obsidian-node/draft`, `POST /api/l15/obsidian-node`, `POST /api/l2b/node/draft`, `POST /api/l2b/node`, `POST /api/l2b/node/delete`, `POST /api/l2b/edge/draft`, `POST /api/l2b/edge`. | `memory_graph_workspace_business_flow_20260513.md` | Default dry-run; Web-only receipts. Node create/update routes through `L15Pool.admit(Observation(source=USER_EXPLICIT))`; delete uses `L15Pool.evict`; real edge connect requires operator mode. Obsidian vault scan/import previews use `UserTagFilter -> L15Pool.admit` only behind operator mode and now return explicit receipt errors for selected profile mismatch, selected over-limit notes, invalid notes, missing paths, and invalid `limit`. |
| Graphiti console | `GET /api/graphiti/status`, `POST /api/graphiti/search`, `POST /api/graphiti/episode/draft`, `POST /api/graphiti/episode`, `POST /api/graphiti/subgraph/search`, `POST /api/graphiti/subgraph/export-draft`, `POST /api/graphiti/subgraph/export`; dry-run first. | `graphiti_management_business_flow_20260513.md` | Implemented safe seed plus `arknights_test`/DeepSeek config and Graphiti-to-L1.5 subgraph export receipts. React Source Board now supports selected-hit subgraph preview and dry-run apply; full Graphiti/FalkorDB surgery remains future operator work. |
| Google Calendar preview | `POST /api/google/calendar/preview`; raw event -> normalized CalendarTrigger event -> `GOOGLE_CALENDAR` Observation preview. | `memory_graph_workspace_business_flow_20260513.md` | Read-only preview; real import remains TriggerOutcome/L1.5 and does not use browser-held Google OAuth. |
