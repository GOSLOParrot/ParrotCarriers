...
| Live memory snapshot | `GET /api/app/live-state?limit=...`, `GET /api/memory/live-state/changes?since=...&limit=...`, `GET /api/memory/blackboard/activity?limit=...`; grouped Blackboard key rows, grouped IntentWorkspace ref rows, Ref registry, React Flow L2-B graph/detail panel, bounded changed-since events, and bounded Blackboard activity rows. | `memory_graph_workspace_business_flow_20260513.md` | Implemented as bounded active-view polling renderer with filters, new-node diff highlight, and soft Intent/Ref-to-L2-B links when linked nodes exist. 2026-05-15 Memory changed-since V1 now lets React skip no-op broad snapshot repainting. This is still polling/diff; SSE/WebSocket remains a later CORE-009 evaluation after the visual model and shared App/Web need are reviewed. |
| L1.5/L2-B operator drafts | `GET /api/l15/pool`, `GET /api/l15/obsidian-vault/scan`, `POST /api/l15/obsidian-vault/import-draft`, `POST /api/l15/obsidian-vault/import`, `POST /api/l15/bucket-op/draft`, `POST /api/l15/bucket-op`, `POST /api/l15/obsidian-node/draft`, `POST /api/l15/obsidian-node`, `POST /api/l2b/node/draft`, `POST /api/l2b/node`, `POST /api/l2b/node/delete`, `POST /api/l2b/edge/draft`, `POST /api/l2b/edge`, `POST /api/l2b/edge/update`, `POST /api/l2b/edge/delete`, `POST /api/refs/binding/draft`, `GET /api/photos/asset/{day}/{photo_id}`. | `memory_graph_workspace_business_flow_20260513.md` | Default dry-run; Web-only receipts. Node create/update routes through `L15Pool.admit(Observation(source=USER_EXPLICIT))`; delete uses `L15Pool.evict`; real edge connect/update/delete requires operator mode. Edge update/delete identify runtime edges by endpoints plus optional kind/source filters instead of exposing RustWorkX edge indexes. Obsidian vault scan/import previews use `UserTagFilter -> L15Pool.admit` only behind operator mode, return explicit selected-note errors, and the React Source Board now renders those receipts as an inline Import plan. Ref/photo binding currently has draft-only CORE-006 receipts and no apply route until the shared RefBinding API is reviewed; `target_kind=unresolved` is represented as an unresolve/clear preview, not a resolve operation. Photo thumbnails are read-only cache-root previews and do not expose arbitrary local paths. |
| Graphiti console | `GET /api/graphiti/status`, `POST /api/graphiti/search`, `POST /api/graphiti/episode/draft`, `POST /api/graphiti/episode`, `POST /api/graphiti/subgraph/search`, `POST /api/graphiti/subgraph/export-draft`, `POST /api/graphiti/subgraph/export`; dry-run first. | `graphiti_management_business_flow_20260513.md` | Implemented safe seed plus `arknights_test`/DeepSeek config and Graphiti-to-L1.5 subgraph export receipts. React Source Board now supports selected-hit subgraph preview, inline Export plan, `edge_drafts`, tolerant search limit parsing, and dry-run apply; full Graphiti/FalkorDB surgery remains future operator work. |
| Google Calendar source import | `POST /api/google/calendar/fetch`, `GET /api/google/calendar/results`, `POST /api/google/calendar/preview`, `POST /api/google/calendar/import-draft`, `POST /api/google/calendar/import`; Scheduler/Nanobot fetch request or raw event -> normalized CalendarTrigger event -> `GOOGLE_CALENDAR` Observation -> optional L1.5 admit. | `memory_graph_workspace_business_flow_20260513.md`, `observability_runtime_business_flow_20260513.md` | Preview first; real fetch/import requires explicit operator mode, preserves event time fields during L1.5 import, returns Web-only mapping rows for L1.5/L2-B target clarity, reads a bounded Scheduler trigger-result ledger without browser-held Google OAuth, and marks cancelled/deleted rows as historical tombstone EVENT nodes rather than evicting by default. |
| Time-aligned evidence debug | `GET /api/vision/evidence/status`, `GET /api/vision/evidence/timeline`, `POST /api/vision/evidence/request`, `POST /api/vision/evidence/stage-hint`, `POST /api/vision/evidence/frame-cache/upload`, `GET /api/vision/evidence/{evidence_id}`, `POST /api/app/test/visual-attention`; React Runtime Time/Evidence panel. | `observability_runtime_business_flow_20260513.md`, `memory_graph_workspace_business_flow_20260513.md` | Web/backend-first CORE-012 prototype. Consumes optional ECP payload/meta `timebase` without changing Unity DTO top-level fields; records photo/snapshot/attention/frame rows; `identify_object` now requests stored evidence instead of old snapshot RPC. `parrot.brain.vision.frame_cache.record_livekit_frame_bytes()` is the storage-backed producer entry point, the Web upload route is only an operator/debug ingress, and `parrot.brain.vision.livekit_sampler` is the Brain room-scoped low-FPS LiveKit track consumer. `/api/vision/evidence/status` now includes secret-free `livekit_sampler` status from the sampler status file plus frame-cache/sampler freshness fields (`latest_frame_age_ms`, `latest_frame_fresh`, per-track summaries including `publication_source`), and `src/scripts/smoke_livekit_frame_sampler.py` is the manual real-room video ingestion smoke. The sampler recognizes camera and screen-share publications, including `SOURCE_SCREEN_SHARE`; React Runtime's `检查采样` button records a read-only receipt that summarizes sampler/frame-cache freshness and likely screen-share evidence. `parrot.brain.vision.evidence_image` prepares local stored images/crops for VLM describe, letting `identify_object` enrich L0/L1 search without inline image transport. `parrot.brain.vision.evidence_awareness` stages `visual_evidence_hint` refs into IntentWorkspace and writes `transient/evidence_awareness_notice`; Photo Awareness writes `transient/photo_awareness_notice` for App menu notification levels; `ContextInjector` now consumes allowed evidence/photo notices as C3 chat-context hints and deliberately keeps C4/speech as a later safe-turn policy. React build uses hashed assets to avoid stale bundle locks on the running console. Live Unity/Web screen-share/LiveKit video smoke, attention-threshold auto evidence bridge, reference-image VLM compare, and C4 safe-turn review remain WEB-015 follow-ups. |
