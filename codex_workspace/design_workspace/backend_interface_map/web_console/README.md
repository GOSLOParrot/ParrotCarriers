...
| Collaboration Flow workbench | `GET /api/runtime/capabilities/catalog`, `GET/POST/DELETE /api/runtime/workflows/drafts`, `GET /api/runtime/workflows/drafts/{workflow_id}`, `POST /api/runtime/workflow/plan-draft`, React Runtime capability catalog/search/filter, durable Web workflow draft registry, trigger draft/fire from inserted nodes, and workflow draft -> HITL Plan import. | `collaboration_flow_workbench_ssot_20260517.md`, `_tmp/collaboration_flow_workplan_20260517.md` | 2026-05-17 first/second slices implemented after workflow/HITL/agent research. Catalog classifies true routes by kind, execution policy, ascent channel, interaction module, information tag, Plan/Nanobot compatibility, result destination, and true-connection state. `plan-draft` creates real HITL Plans only for Nanobot-compatible capabilities and can now import by saved `workflow_id`. React Runtime can save/load/delete Web-only workflow drafts; storage redacts likely secret/token/API-key fields. Trigger nodes execute through existing operator-gated trigger routes. Local 7893 true smoke with ECS Graphiti proxy proved capability catalog, durable draft save/list/get/delete, workflow-id Plan import, trigger draft, Graphiti status, `noble_etiquette` subgraph search, and `selected_hits` import-plan preview. ECS `8790` app-monitor now exposes the same runtime workflow subset for remote smoke parity. Shared Scheduler workflow storage, result-routing contract, and trigger/message HITL state machines remain future work. |
