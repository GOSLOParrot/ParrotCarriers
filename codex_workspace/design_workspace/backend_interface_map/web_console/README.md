...
| Collaboration Flow workbench | `GET /api/runtime/capabilities/catalog`, `POST /api/runtime/workflow/plan-draft`, React Runtime capability catalog/search/filter, local workflow draft, trigger draft/fire from inserted nodes, and workflow draft -> HITL Plan import. | `collaboration_flow_workbench_ssot_20260517.md`, `_tmp/collaboration_flow_workplan_20260517.md` | 2026-05-17 first slice implemented after workflow/HITL/agent research. Catalog classifies true routes by kind, execution policy, ascent channel, interaction module, information tag, Plan/Nanobot compatibility, result destination, and true-connection state. `plan-draft` creates real HITL Plans only for Nanobot-compatible capabilities; trigger nodes execute through existing operator-gated trigger routes. Local 7893 true smoke with ECS Graphiti proxy proved capability catalog, nested workflow plan draft, trigger draft, Graphiti status, `noble_etiquette` subgraph search, and `selected_hits` import-plan preview. Durable workflow storage, result-routing contract, and trigger/message HITL state machines remain future work. |
