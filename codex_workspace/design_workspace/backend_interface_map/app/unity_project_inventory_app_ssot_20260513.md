...
| `LiveKit/MicrophonePublisher.cs` simple phone-mic default | Formal demo audio baseline. | As of 2026-05-18, `simplePhoneMicMode=true` is the default formal App path. It intentionally behaves like the proven ParrotDev path: one Unity `MicrophoneSource`, default phone microphone, fixed `simple_phone_mic_48k`, no Bluetooth permission request, no Android communication-mode request, no route-change republish subscription, no Android `AudioRecord` promotion, and no Settings-page mic cycling. The previous Bluetooth/SCO/AudioRecord ladder remains in code only behind `simplePhoneMicMode=false` and is a future route-lab feature, not the current phone demo baseline. HUD/menu must show this as `simple phone mic / route lab off` so tests do not mistake the stable baseline for production Bluetooth routing. |
| `Core/AndroidRuntimePermissions.cs` | Formal Android runtime permission helper. | Startup, microphone publish, route diagnostics, and Android AudioRecord startup must use this shared helper for `RECORD_AUDIO` on Android. On Android players it treats `UnityEngine.Android.Permission.HasUserAuthorizedPermission("android.permission.RECORD_AUDIO")` as the only truth; Unity's legacy `Application.HasUserAuthorization(UserAuthorization.Microphone)` is logged for diagnostics but must not be OR'ed into the Android gate. This exists because iQOO targetSdk 36 evidence showed the package can be `RECORD_AUDIO: ignore` / `granted=false` while Unity's legacy state may look authorized, causing fake `Mic wait` states with no usable input. |
...
communication state.
After the 2026-05-18 iQOO evidence, `auto`, `system_default`, and `phone_mic`
are treated as unconditional media-mode capture preferences: they keep
`MODE_NORMAL`, clear stale speakerphone pins, and rely on the App-owned
`AudioRecord`/phone MIC path. Explicit Bluetooth microphone/SCO is deferred to
a manual production control and must not be silently selected by `auto`.
Route initialization also forces both the Java singleton and C# facade back to
observe/media mode, because Android process state can outlive a Unity scene or
failed publish. The App-owned Android `AudioRecord` source is a LiveKit custom
PCM source, not `AudioSourceMicrophone`; each retry sets both SDK sample-rate
defaults before constructing its local source.
