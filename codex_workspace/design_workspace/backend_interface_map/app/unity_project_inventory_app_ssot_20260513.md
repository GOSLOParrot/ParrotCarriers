...
| `LiveKit/MicrophonePublisher.cs` | Formal microphone track executor. | Publishes the LiveKit mic track, owns the audio health producer, requests microphone and Android 12+ Bluetooth-connect permission when voice capture starts, and rebuilds the source on accepted `AudioRouteManager` changes. It does not reconnect the LiveKit room for route changes. `AudioRouteDetector` remains fallback/diagnostic only. Current Settings-page `MIC NEXT` / `MIC AUTO` local device-name controls are diagnostic/debug only. A2DP remains output-only and keeps the normal 48 kHz microphone policy; only SCO is treated as a Bluetooth microphone route and uses the 16 kHz capture policy. If native Android routing is available, microphone permission is granted, and Unity `Microphone.devices` is empty, the executor may use `android_default_microphone` by passing `null` to the LiveKit/Unity microphone source; this is the formal phone-mic/default communication-input fallback, not a Smoke-script rule. `MIC NEXT` mirrors that fallback as `auto:android_default_microphone` instead of reporting a stuck no-device state. Manual preference is local Unity runtime state, not RoomSetting or Brain policy truth. |
