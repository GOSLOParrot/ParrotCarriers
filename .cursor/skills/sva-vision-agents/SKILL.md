---
name: sva-vision-agents
description: Use when working with Vision-Agents Processor pattern (VideoProcessor, attach_agent, add_frame_handler, fps, Gemini Realtime integration)
---
...
*   **Context injection via Processor 鈫?Agent event system**: Processors analyze media and inject their findings (e.g., detected objects, poses) into the agent's event stream, which the LLM can then consume as part of its conversational context.
