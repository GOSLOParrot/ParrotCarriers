...
| L9 | Δ 权重 + 阈值器位置 | **阈值器在 `dsg/attention/threshold.py`，不塞 BB**。BB 只放结果（`transient/current_attention_hint`）。**Phase 4 起步数值**：Δ_focus = 0.2，Δ_bbox = 1.0，threshold = 1.0（**1 BBox 直接到阈值**；**5 Focus 累加到阈值**）。**初始值入菜单** = Unity ScriptableObject `ParrotAttentionConfig`（参考 `ParrotLifecycleConfig` 模式），Brain 端通过 BB key `global/attention_thresholds`（# CANDIDATE）由 Unity Echo 注入。注：起步数值是"达阈值即上报"的最小可工作组合，W6-7 真机调时再细调（如 Δ_bbox 是否要 < threshold 给 release 留空间）。<br/><br/>**⚠ Echo 路径 ①+② 已接通（Unity B chat, 2026-04-30）**：Phase 4 W6-7 Unity 半边落地后，Echo 链路前两步已实质化：① Unity `ParrotAttentionConfig` SO + `AttentionConfigEchoPublisher`（`unity/ArSpike/Assets/Scripts/ParrotApp/Attention/`）在 `RoomManager.OnConnected`（含 reconnect / Brain 管线切换）publish EcpEvent `attention.config.echo` → ② Brain `attention_config_handler.py`（subscribe via `event_ingest`）写 BB `global/attention_thresholds`（writer 字符串保持 `brain._rpc_bridge` 与 bb_schema 声明一致）。**剩余 prerequisite（仍 deferred）**：③ `FocusBboxThreshold.__init__` 在 `register()` 前读 BB 注入构造参数 — 触动 `dsg/attention/threshold.py`，当前 chat 锁定，留给独立 Brain chat 的 1 行改动。① + ② 落地 = BB key 已有真 producer，但 `bb_schema.py` 的 # CANDIDATE 标记保留至 ③ 完成（doc-only follow-up 升级） |
