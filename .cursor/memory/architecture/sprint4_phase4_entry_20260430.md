...
| L9 | Δ 权重 + 阈值器位置 | **阈值器在 `dsg/attention/threshold.py`，不塞 BB**。BB 只放结果（`transient/current_attention_hint`）。**Phase 4 起步数值**：Δ_focus = 0.2，Δ_bbox = 1.0，threshold = 1.0（**1 BBox 直接到阈值**；**5 Focus 累加到阈值**）。**初始值入菜单** = Unity ScriptableObject `ParrotAttentionConfig`（参考 `ParrotLifecycleConfig` 模式），Brain 端通过 BB key `global/attention_thresholds` 由 Unity Echo 注入。注：起步数值是"达阈值即上报"的最小可工作组合，W6-7 真机调时再细调（如 Δ_bbox 是否要 < threshold 给 release 留空间）。<br/><br/>**✅ Echo 全链路接通（2026-04-30）**：① Unity `ParrotAttentionConfig` SO + `AttentionConfigEchoPublisher`（`unity/ArSpike/Assets/Scripts/ParrotApp/Attention/`）在 `RoomManager.OnConnected`（含 reconnect / Brain 管线切换）publish EcpEvent `attention.config.echo` → ② Brain `attention_config_handler.py`（subscribe via `event_ingest`）写 BB `global/attention_thresholds`（writer = `brain._rpc_bridge` 与 bb_schema 声明一致）→ ③ `FocusBboxThreshold.__init__` 用 sentinel-None resolution 读 BB 覆盖 DEFAULTS（F-05 step ③）。优先级：**explicit kwargs > BB > DEFAULT_***（sentinel-None 默认参数让生产 boot 自动用 BB，测试可显式 pin DEFAULT 值绕开 BB 状态）。`bb_schema.py` 的 # CANDIDATE 标记已移除 |
...
| `attention.config.echo` | unity | `RoomManager.OnConnected`（含 reconnect / Brain 管线切换）+ Inspector ContextMenu 兜底 | `delta_focus` / `delta_bbox` / `threshold` / `target_ttl_s` / `schema_version=1` | ③（F-05 fix） |
